...
(function($){
  "use strict";
	...
	var Resumo = {
		...
		modal: function(){
			var self		= this;
			var modalBox	= $('.webplace_modalbox');
			var item		= $('.modal_item');
		...
		modal_prevnext: function(prevNext,modalBox){
			...
			prevNext.find('a').off().on('click',function(){
				...
				$(".webplace_modalbox .modal_content").stop().animate({scrollTop:0}, 500, 'swing');
		...
		scrollToAnchor: function(){
			$('a[href^="#"]').not('[href="#"]').not('[href^="#tab"]').on('click',function(){
				...
				if(section.length){
					...
					$('.webplace_wrapper').removeClass('nav-opened nav-hover-close');
					$('.webplace_navigation .nav_footer').removeClass('ready');
					return false;
				}
			});
			$('.webplace_totop').on('click',function(){
				$("html, body").animate({ 
					scrollTop: 0
				}, 1500);
			});
		...
		contactForm: function(){
			$('#send_message').on('click', function(){
				var form		= $('.webplace_contact .contact_form');
				var name 		= $("#name").val();
				var email 		= $("#email").val();
		...
		movingPlaceholder: function(){
			$('.webplace_contact .input_wrapper').each(function(){
				var e		= $(this);
		...
		loadBlogPosts: function(){
			$('.webplace_blog_list .load_more a').on('mousedown',function(){
				var element 	= $(this);
			...
			$('.webplace_blog_list .load_more a').on('click',function(){
				var element 	= $(this);
				var text 		= element.find('.text');
				...
				setTimeout(function(){
					var listItem = element.closest('.webplace_blog_list').find('.be_animated');
		...
		testimonialCarousel: function(){
			var owl 		= $('.webplace_testimonials .owl-carousel');
			owl.each(function(){
				var el 		= $(this);
				var parent	= el.closest('.webplace_testimonials');
				el.owlCarousel({
					autoplay: true,
		...
		progress: function(){
			$('.webplace_progress_bar').each(function() {
				var pWrap 	= $(this);
		...
		tabs: function(){
			$('.webplace_tabs .tab_header a').off().on('click',function(){
				var e 			= $(this);
				var li			= e.parent();
				var tabs		= e.closest('.webplace_tabs');
		...
		rightNav: function(){
			var closer 		= $('.webplace_navigation .closer,.webplace_nav_overlay');
			var overlay		= $('.webplace_nav_overlay');
			var opener 		= $('.webplace_right .menu_trigger');
			var wrapper 	= $('.webplace_wrapper');
			var navFooter	= $('.webplace_navigation .nav_footer');
			var navLi		= $('#nav ul li');
		...
		rightPanelScroll: function(){
			var rightPanel 	= $('.webplace_right .right_in');
			var navIn 		= $('.webplace_navigation .nav_in');
			var nav 		= $('#nav');
			var navFooter	= $('.webplace_navigation .nav_footer');
